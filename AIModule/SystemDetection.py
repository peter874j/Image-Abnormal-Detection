...
def save_video(startTime, q): 
    ...
    outVideo = cv2.VideoWriter('record/{0}_{1}.avi'.format(date, startTime), fourcc, fps, (width, height))
...
class Hands:
    ...
    def run(self, image):
        ...
        if len(yoloResult['PCB'])>0 and len(yoloResult['PCB'])>0:
            streamImg, self.NGFlag, centerFlag = self.check_hands_touch(streamImg, yoloResult['PCB'], yoloResult['Gloves'], self.pixelThres)
        else:
            self.NGFlag = False
            centerFlag = False
    ...
    def check_hands_touch(self, image, PCBBoxes, GlovesBoxes, pixelThres):
